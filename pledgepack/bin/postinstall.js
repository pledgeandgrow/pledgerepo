...
// Uses ESM imports (no require.resolve).
...
function tryBuildFromSource() {
  ...
  // Try to find the source repo — check if we're inside the monorepo
  const possibleSourceDirs = [
    join(__dirname, '..'),           // pledgepack/ within monorepo
    join(__dirname, '..', '..'),     // nested install
  ];
  ...
  for (const dir of possibleSourceDirs) {
    if (existsSync(join(dir, 'Cargo.toml'))) {
      ...
      if (result.status === 0) {
        console.log('  \x1b[32m\u2713\x1b[0m Built from source successfully');
        return;
      }
    }
  }
